...
class Auth
{
	...
	{
		return (int) $this->ci->session->userdata('employee_id');

	}
	function institute_id()
	{
		return (int) $this->ci->session->userdata('institute_id');
	...
	function campus_id()
	{
		return (int) $this->ci->session->userdata('campus_id');

	}
	function default_ims_session_id()
	{
		return (int) $this->ci->session->userdata('default_ims_session_id');

	}
	function institute_type($camufladge=false)
	{
	   if($camufladge)
       {
            return 'school';
       }
	   if($this->ci->session->userdata('institute_type')!='')
       {
            return $this->ci->session->userdata('institute_type');
       }
    ...
    function in_type_id()
	{
		return  $this->ci->session->userdata('in_type_id');

	}
	function criteria()
	{
		$criteria=array();
		$criteria['institute_id']=$this->institute_id();
		$criteria['campus_id']=$this->campus_id();
		//print_r($criteria);
		return $criteria;
	}
	function has_campus($campus_id)
    {
        $campuses=array_keys($this->show_campuses());
        if(in_array($campus_id,$campuses) or $this->campus_id()==$campus_id)
        {
            return TRUE;
        }
        else
	...
	function show_campuses()
    {
        $campuses=array();
        $campuses=$this->ci->session->userdata('show_campuses');
        $campuses[$this->campus_id()]=$this->campus_id();
	...
	private function delete_autologin()
	{
		...
		//$this->ci->db->delete('user_cookies');
        $this->ci->db->query('DELETE FROM `user_cookies` WHERE `created_on`  <  DATE_SUB(CURDATE(), INTERVAL 2 MONTH)');
	 ...
	 */
	function setup_session($user_id=0, $username='', $password_hash=NULL, $email='', $role_id=0, $remember=FALSE, $old_token=NULL,$user_name='')
	{
    	...
    	$in_type_id=$user['in_type_id'];
		$type=$user['type'];
       // var_dump($user);
        //echo $user['institute_id'];
		if($in_type_id!='')
		{
			...
			$data['in_type_id']		=		$user['in_type_id'];
			...
			$query="
				...
				AND		campus_id='".$data['campus_id']."'
				AND		status='default'
				";
		$result=mysql_query($query);
		$ims_session_array=mysql_fetch_array($result);
		$default_ims_session_id=$ims_session_array['id'];
		if($default_ims_session_id!='')
		{
			
			$data['default_ims_session_id']=$default_ims_session_id;
	 	...
	 	$query="
				...
				";
		$result=mysql_query($query);
		$institutes_array=mysql_fetch_array($result);
		$institute_type=$institutes_array['type'];
