...
class Settings extends Admin_Controller
{
	...
	public function __construct()
    {
        ...
        $this->load->model('roles/my_role_model');
        $this->load->model('users/my_user_model');
	...
	public function create()
	{
        ...
        //$this->load->model('dishes/qrcode_model', null, true);
        ...
        $controller_role_id=$this->my_role_model->get_controller_role($role_id);
        
        Template::set('role_id', $role_id);
        Template::set('controller_users', $this->my_user_model->get_users_by_role($controller_role_id));
        Template::set('users', $this->my_user_model->get_all_users());
		Template::set('languages', unserialize($this->settings_lib->item('site.languages')));

		Template::set('toolbar_title', lang('us_create_user'));
		Template::set_view('settings/smart_kitchen');
		Template::render();
...
//        $this->load->model('dishes/qrcode_model', null, true);
		...
		if (in_array('submit_button',array_keys($_POST)))
		{
	...
	private function save_user($type='insert', $id=0, $meta_fields=array(), $cur_role_name = '')
	{
		...
		if ($use_usernames and false)
		{
			$extra_unique_rule = $type == 'update' ? ',users.id' : '';

			//$this->form_validation->set_rules('username', lang('bf_username'), 'required|trim|strip_tags|unique[users.username'.$extra_unique_rule.']|xss_clean');
		}
    ...
    //		$this->form_validation->set_rules('timezones', lang('bf_timezone'), 'required|trim|strip_tags|max_length[4]|xss_clean');
     ...
     //   $this->qrcode_model->generate_qr_code($id,$id,'users');
    ...
    function    upload_image($id)
    {
        ...
        $upload_location = $this->my_user_model->get_user_image_upload_location();
